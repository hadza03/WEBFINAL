...
Flight::route('POST /customers/add', function() {
    ...
    */
    ...
    if (!isset($data['first_name'], $data['last_name'], $data['birth_date'])) {
...
Flight::route('GET /foods/report', function(){
    ...
    */
    ...
    $pagedFoods = array_slice($foods, $offset, $perPage);

    $result = array_map(function($food) {
        return [
            'name' => $food['name'],
            'brand' => $food['brand'],
            'image' => '<img src="' . htmlspecialchars($food['image_url']) . '" alt="' . htmlspecialchars($food['name']) . '">',
            'energy' => $food['energy'],
